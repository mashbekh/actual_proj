...
public class CompanyServiceImpl {
			...
			@FormDataParam("logo") InputStream logoDoc, @QueryParam("id") String id, @QueryParam("token") String token) throws IOException, AppException, Exception, Throwable, EntityException, AppException
	{
		...
		ColumnSettings column = new ColumnSettings("Products",null,"Quantity", null,"Price",null);

		CompanyDaoImpl companydao = new CompanyDaoImpl();
		Company company = new Company(bd,cd,td,adList,po,estimate,invoice,column,null);
		String  companyId = companydao.createCompany(company,id);


		return Response.status(Response.Status.OK)
				.entity(companyId).build();
	}
	...
	@Path("/get")
	@Produces(MediaType.APPLICATION_JSON)
	public Response getCompany(@QueryParam("cid") String companyId, @QueryParam("id") String userId, @QueryParam("token") String token) throws IOException, AppException, Exception, Throwable, EntityException, AppException
	{
		CompanyDaoImpl companydao = new CompanyDaoImpl();
		Company cmp  = companydao.getCompany(companyId);
		return Response.status(Response.Status.OK)
				.entity(cmp).build();
	}
	...
	public Response updateFormatSettings(@QueryParam("po") String poSettings, @QueryParam("estimate") String estimateSettings, @QueryParam("invoice") String invoiceSettings, @QueryParam("column") String colSettings ,@QueryParam("cid") String companyId, @QueryParam("id") String userId, @QueryParam("token") String token) throws JsonMappingException , JsonParseException, IOException, AppException, Exception, Throwable, EntityException, AppException
	{
		...
		InvoiceSettings inv = objectMapper.readValue(invoiceSettings, InvoiceSettings.class);
		
		ColumnSettings col = objectMapper.readValue(colSettings, ColumnSettings.class);
		
		CompanyDaoImpl companydao = new CompanyDaoImpl();
		Company cmp = companydao.updateFormatSettings(po,es,inv,col,companyId);
