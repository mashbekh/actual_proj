...
public class VendorDaoImpl {
	...
	public BusinessPlayers createVendor(BusinessPlayers bplayer , String companyId ) throws EntityException
	{
		Datastore ds = null;
		Company cmp = null;
	...
	public List<BusinessPlayers> getVendors(String companyId) throws EntityException
	{
		Datastore ds = null;
		Company cmp = null;
		ObjectId oid = null;
		List<BusinessPlayers> vendorList = new ArrayList<>();
		...
		try
		{
			...
			Query<BusinessPlayers> vendorquery = ds.createQuery(BusinessPlayers.class).field("company").equal(cmp).field("isVendor").equal(true).field("isDeleted").equal(false);
			vendorList = vendorquery.asList();
		...
		catch(MongoException e)
		{
			...
			if(cmp != null && vendorList.isEmpty() == true)
